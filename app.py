...
def verify_turnstile(token, remote_ip=None):
    """
    ตรวจสอบ token จาก Cloudflare Turnstile
    ถ้าไม่ได้ตั้งค่า key ไว้ ให้ถือว่าผ่านอัตโนมัติ (กันล็อกอิน/สมัครไม่ได้)
    """
...
def inject_globals():
    # ให้ใช้ตัวแปร TURNSTILE_SITE_KEY ได้ในทุก template
...
EMAIL_GMAIL_PATTERN = re.compile(r"^[A-Za-z0-9._%+-]+@gmail\.com$")
...
def is_valid_gmail(email: str) -> bool:
    ...
    return bool(EMAIL_GMAIL_PATTERN.match(email))
...
def send_email(to_email: str, subject: str, body: str) -> None:
    ...
    ส่งอีเมลแบบง่าย ๆ
    ลำดับการทำงาน:
      1) ถ้ามี RESEND_API_KEY + SMTP_FROM -> ส่งผ่าน Resend API
      2) ถ้ามีค่า SMTP_HOST -> ส่งผ่าน SMTP ปกติ
      3) ถ้าไม่มีทั้งคู่ -> พิมพ์ลง log (MOCK EMAIL) เพื่อทดสอบ
    """
    ...
    from_email = os.environ.get("SMTP_FROM") or os.environ.get("RESEND_FROM") or os.environ.get("SMTP_USER")

    # 1) พยายามส่งผ่าน Resend API ก่อน (ถ้ามีการตั้งค่า)
    if resend_key and from_email:
        try:
            ...
            if resp.status_code in (200, 201, 202):
                ...
                print("ส่งอีเมลผ่าน Resend ไม่สำเร็จ:", resp.status_code, resp.text)
        except Exception as e:
            print("ข้อผิดพลาดขณะส่งอีเมลผ่าน Resend:", e)

    # 2) ถ้ามี SMTP_HOST ให้ลองส่งผ่าน SMTP เป็น fallback
    ...
    if smtp_host and from_email:
        try:
            ...
            print("ส่งอีเมลผ่าน SMTP ไม่สำเร็จ:", e)

    # 3) ถ้าไม่มีทั้ง Resend และ SMTP ให้พิมพ์ลง log สำหรับทดสอบ
    print("=== MOCK EMAIL (ยังไม่ได้ตั้งค่า RESEND_API_KEY หรือ SMTP) ===")
    print("To:", to_email)
...
def send_password_reset_email(to_email: str, code: str) -> None:
    """ส่งอีเมลรหัสกู้คืนรหัสผ่านให้ผู้ใช้"""
    to_email = normalize_email(to_email)
    subject = "MySeriesVideo - รหัสกู้คืนบัญชีของคุณ"
    body = f"""
สวัสดีจาก MySeriesVideo

มีการขอกู้คืนรหัสผ่านสำหรับบัญชีที่ใช้อีเมลนี้

รหัสยืนยันของคุณคือ: {code}

กรุณานำรหัส 6 หลักนี้ไปกรอกในหน้ากู้คืนบัญชีภายใน 15 นาที
หากคุณไม่ได้เป็นคนส่งคำขอนี้ สามารถเพิกเฉยอีเมลฉบับนี้ได้

ขอบคุณที่ใช้งาน MySeriesVideo
© 2025 MySeriesVideo
""".strip()
...
def ensure_user_extra_columns(conn: sqlite3.Connection):
    """เพิ่มคอลัมน์ user_key, plain_password และ email ให้ตาราง users ถ้ายังไม่มี
...
def user_register():
    ...
    if request.method == "POST":
        ...
        if not username or not email or not password or not password_confirm:
            ...
            flash("ระบบรองรับเฉพาะอีเมล @gmail.com กรุณาใส่ @gmail.com ต่อท้ายอีเมล", "error")
            ...
            try:
                ...
                if existing_email:
                    ...
                    conn.execute(
                        "INSERT INTO users (username, email, password, plain_password, user_key, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                        (username, email, hashed, password, user_key, datetime.utcnow().isoformat()),
                    )
                    conn.commit()
...
def forgot_password():
    """
    ขั้นตอนที่ 1: กรอกอีเมลที่ใช้สมัคร เพื่อขอรหัสกู้คืน 6 หลักทางอีเมล
    """
    if request.method == "POST":
        email = normalize_email(request.form.get("email", ""))

        if not email:
            flash("กรุณากรอกอีเมลที่ใช้สมัคร", "error")
        elif not is_valid_gmail(email):
            flash("โปรดใส่ @gmail.com ต่อท้ายอีเมล (ระบบรองรับเฉพาะ @gmail.com)", "error")
        else:
            conn = get_db_connection()
            user = conn.execute(
            ...
            if user is None:
                ...
                conn.execute(
                    "DELETE FROM password_resets WHERE user_id = ? OR datetime(expires_at) < datetime(?)",
                    (user["id"], now.isoformat()),
                )
                conn.execute(
                    """
                    INSERT INTO password_resets (user_id, code, created_at, expires_at, used)
                    VALUES (?, ?, ?, ?, 0)
                    """,
                    (user["id"], code, now.isoformat(), expires_at.isoformat()),
                )
                conn.commit()
                conn.close()
                ...
                flash("ส่งรหัสกู้คืนไปที่อีเมลเรียบร้อยแล้ว กรุณาตรวจสอบกล่องจดหมาย", "success")
...
def forgot_password_verify():
    """
    ขั้นตอนที่ 2: กรอกรหัส 6 หลักที่ได้รับทางอีเมล
    """
    user_id = session.get("reset_user_id")
    if not user_id:
        flash("เซสชันหมดอายุ กรุณากรอกอีเมลใหม่อีกครั้ง", "error")
        return redirect(url_for("forgot_password"))

    if request.method == "POST":
        code = (request.form.get("code", "") or "").strip()
        if not re.fullmatch(r"\d{6}", code):
            flash("กรุณากรอกรหัส 6 หลักให้ถูกต้อง", "error")
        else:
            conn = get_db_connection()
            row = conn.execute(
                """
            ...
            if row is None:
                conn.close()
                flash("รหัสกู้คืนไม่ถูกต้อง", "error")
            else:
                now = datetime.utcnow()
                try:
                ...
                if now > expires_at:
                    conn.close()
                    flash("รหัสกู้คืนหมดอายุแล้ว กรุณาขอรหัสใหม่อีกครั้ง", "error")
                else:
                    conn.execute(
                    ...
                    flash("ยืนยันรหัสกู้คืนเรียบร้อยแล้ว กรุณาตั้งรหัสผ่านใหม่", "success")
...
def forgot_password_reset():
    """
    ขั้นตอนที่ 3: ตั้งรหัสผ่านใหม่ หรือกดข้ามไปหน้าเข้าสู่ระบบ
    """
    user_id = session.get("reset_verified_user_id")
    if not user_id:
        flash("เซสชันหมดอายุ กรุณากรอกอีเมลและรหัสกู้คืนใหม่อีกครั้ง", "error")
        return redirect(url_for("forgot_password"))
    ...
    if request.method == "POST":
        ...
        if not new_password or not confirm_password:
            flash("กรุณากรอกรหัสผ่านใหม่ให้ครบ", "error")
            ...
            flash("เปลี่ยนรหัสผ่านใหม่สำเร็จแล้ว กรุณาเข้าสู่ระบบอีกครั้ง", "success")
            return redirect(url_for("user_login"))
...
def user_account():
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            if not new_email:
                flash("กรุณากรอกอีเมลใหม่", "error")
                ...
                if existing_email:
                    ...
                    conn.commit()
                    conn.close()
                    flash("เปลี่ยนอีเมลสำเร็จ", "success")
                    return redirect(url_for("user_account"))
...
def admin_users():
    ...
    if q:
        like = f"%{q}%"
        users = conn.execute(
            "SELECT * FROM users WHERE username LIKE ? OR user_key LIKE ? OR email LIKE ? ORDER BY datetime(created_at) DESC",
            (like, like, like),
        ).fetchall()
    else:
        users = conn.execute(
                ...
                for u in users_list:
                    ...
                    if existing:
                        cur.execute(
                            """
                            UPDATE users
                            SET username = ?, email = ?, password = ?, plain_password = ?, user_key = ?, created_at = ?
                            WHERE id = ?
                            """,
                            (
                                username,
                        ...
                        cur.execute(
                            """
                            INSERT INTO users (id, username, email, password, plain_password, user_key, created_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                            (
                                uid,
...
def admin_backup_download_videos():
    ...
    filename = f"myseries_videos_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    return Response(
        json_bytes,
...
def admin_backup_download_users():
    ...
    json_bytes = json.dumps(data, ensure_ascii=False, indent=2).encode("utf-8")
    filename = f"myseries_users_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    return Response(
        json_bytes,
        mimetype="application/json",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
...
def admin_backup_download_other():
    ...
    json_bytes = json.dumps(data, ensure_ascii=False, indent=2).encode("utf-8")
    filename = f"myseries_other_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    return Response(
        json_bytes,
        mimetype="application/json",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
