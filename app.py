...
def ensure_announcements_table(conn: sqlite3.Connection):
    cur = conn.cursor()
    cur.execute(
        """
    ...
    if request.method == "POST":
        ...
        if not username or not password or not password_confirm:
            ...
            try:
                ...
                conn.close()
                login_user(user)
                flash("สมัครบัญชีสำเร็จ และเข้าสู่ระบบแล้ว", "success")
                return redirect(url_for("index"))
...
def admin_announcements():
    ...
    if request.method == "POST":
        ...
        if image_file and image_file.filename:
            filename = datetime.utcnow().strftime("%Y%m%d%H%M%S_") + secure_filename(image_file.filename)
            upload_dir = os.path.join(app.root_path, "static", "uploads", "announcements")
            os.makedirs(upload_dir, exist_ok=True)
            file_path = os.path.join(upload_dir, filename)
            image_file.save(file_path)
            final_image_url = url_for("static", filename=f"uploads/announcements/{filename}")

        if not title:
            flash("กรุณากรอกหัวข้อประกาศ", "error")
        else:
            conn.execute(
                "INSERT INTO announcements (title, content, image_url, created_at) VALUES (?, ?, ?, ?)",
                (title, content, final_image_url, datetime.utcnow().isoformat()),
            )
            conn.commit()
...
def admin_edit_announcement(ann_id):
    ...
    if request.method == "POST":
        ...
        if not title:
            ...
            conn.execute(
                "UPDATE announcements SET title = ?, content = ?, image_url = ? WHERE id = ?",
                (title, content, final_image_url, ann_id),
            )
            conn.commit()
            flash("บันทึกการแก้ไขประกาศแล้ว", "success")

        conn.close()
        return redirect(url_for("admin_announcements"))

    conn.close()
    return render_template("admin_edit_announcement.html", announcement=ann)
