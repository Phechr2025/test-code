...
def ensure_user_key_column(conn: sqlite3.Connection):
    """เพิ่มคอลัมน์ user_key ให้ตาราง users ถ้ายังไม่มี (ใช้ค้นหา / รีเซ็ต key)."""
...
def generate_user_key(length: int = 12) -> str:
    """สร้างคีย์สำหรับผู้ใช้แบบสุ่ม (ใช้ตัวอักษรใหญ่ + ตัวเลข)."""
    ...
    if request.method == "POST":
        ...
        if not username or not password or not password_confirm:
            ...
            try:
                ...
                conn.execute(
                    "INSERT INTO users (username, password, created_at, user_key) VALUES (?, ?, ?, ?)",
                    (username, hashed, datetime.utcnow().isoformat(), generate_user_key()),
                )
                conn.commit()
                user = conn.execute(
...
def user_account():
    ...
    if request.method == "POST":
        ...
        if action == "reset_key":
            ...
            conn.commit()
            conn.close()
            flash("รีเซ็ต key ใหม่เรียบร้อยแล้ว", "success")
...
def admin_users():
    ...
    if q:
        ...
        users = conn.execute(
            "SELECT * FROM users ORDER BY datetime(created_at) DESC LIMIT 200"
        ).fetchall()
    conn.close()
    ...
    if request.method == "POST":
        ...
        if not new_username:
            ...
            try:
                ...
                conn.commit()
                flash("บันทึกข้อมูลผู้ใช้เรียบร้อยแล้ว", "success")
            except sqlite3.IntegrityError:
                flash("ชื่อผู้ใช้นี้มีอยู่ในระบบแล้ว", "error")
...
@app.route("/admin/users/<int:user_id>/reset_key", methods=["POST"])
def admin_user_reset_key(user_id: int):
    if not admin_required():
        return redirect(url_for("admin_login"))
    ...
    conn.commit()
    conn.close()
    flash("รีเซ็ต key ใหม่ของผู้ใช้เรียบร้อยแล้ว", "success")
    return redirect(url_for("admin_users", q=user["username"]))
...
def admin_user_delete(user_id: int):
    ...
    conn.commit()
    conn.close()
    flash("ลบบัญชีผู้ใช้และประวัติการดูเรียบร้อยแล้ว", "success")
